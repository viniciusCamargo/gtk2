...
int main(int argc, char *argv[]) {
  ...
  toggle1 = gtk_toggle_button_new_with_mnemonic("Deactivate the other one!");
  toggle2 = gtk_toggle_button_new_with_mnemonic("No! Deactivate that one!");

  g_signal_connect(
    G_OBJECT(toggle1), "toggled",
    G_CALLBACK(button_toggled),
    (gpointer) toggle2
  );

  g_signal_connect(
    G_OBJECT(toggle2), "toggled",
    G_CALLBACK(button_toggled),
    (gpointer) toggle1
  );
