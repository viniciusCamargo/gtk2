...
int main(int argc, char *argv[]) {
  ...
  child1 = gtk_label_new("Go to page 2");
  child2 = gtk_label_new("Go to page 1");

  /* Notice that two widgets were connected to the same callback function */
  g_signal_connect(
    G_OBJECT(child1), "clicked",
    G_CALLBACK(switch_page),
    (gpointer) notebook
  );

  g_signal_connect(
    G_OBJECT(child2), "clicked",
    G_CALLBACK(switch_page),
    (gpointer) notebook
  );
  ...
  /* Set the position of the tabs in the window */
